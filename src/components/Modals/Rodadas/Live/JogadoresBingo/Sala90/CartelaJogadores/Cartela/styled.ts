...
export const CartelaNumbers = styled.div<CartelaNumbersProps>`
  ...
  border: 3px solid
    ${({ isMarked, isDisabled }) => {
      if (isMarked) return '#FF2222';
      if (isDisabled) return '#747373';
      return '#747373';
    }};
  background-color: ${({ isMarked, isDisabled }) => {
    if (isMarked) return '#FFDDDD';
    if (isDisabled) return '#A2A2A2';
    return 'transparent';
  }};
  font-family: "Primary";
  font-size: 24px;
  font-weight: bold;
  color: ${({ isMarked, isDisabled }) => {
    if (isMarked) return '#FF2222';
    if (isDisabled) return '#A2A2A2';
    return '#747373';
  }};
